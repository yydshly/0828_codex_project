...
def commit_render(
    ...
    try:
        ...
        for index, (canvas, expected_hash) in enumerate(zip(canvases, expected_hashes)):
            ...
            # Windows rejects fsync on a read-only descriptor. The staged PNG is
            # already flushed and closed by save_png; keep the upstream durability
            # check on POSIX and continue with byte/hash verification on Windows.
            if os.name != "nt":
