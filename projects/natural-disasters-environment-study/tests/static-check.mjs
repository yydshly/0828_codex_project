...
const checks = [
  ...
  ['design contract supersedes Revision 3 and records remote-delivery revision', contract.includes('Current authority: Revision 8') && contract.includes('GitHub Pages delivery')],
  ...
  ['Pages build publishes corrected evidence', buildScript.includes('upstream-runtime-observation.json') && buildScript.includes('runtime-correction.md') && buildScript.includes('web-workbench-validation.md')],
  ['project test scripts target corrected checks', packageJson.scripts['test:project-011']?.includes('static-check.mjs') && packageJson.scripts['test:project-011:upstream-browser']?.includes('upstream-runtime-check.mjs') && packageJson.scripts['test:project-011:web-browser']?.includes('web-workbench-check.mjs')],
