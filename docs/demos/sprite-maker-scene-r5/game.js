...
(() => {
  ...
  const framePaths = [1, 2, 3, 4].map((index) => `../../assets/project-009-game/lin-jian-idle-${String(index).padStart(2, '0')}.png`);
  const npcFrames = [];
  const input = { left: false, right: false };

  const terminalDefinitions = [
    { id: 'rig', x: 760, color: '#6cabff', label: 'RIG', title: '01 / Rig 与确定性四帧', description: '同一个身份 master，经视觉骨架建议与本地 Rust 变形，生成四张可重复的透明 PNG。' },
  ...
  function drawNpc() {
    ...
    ctx.font = '700 11px "Cascadia Mono", monospace';
    ctx.textAlign = 'center';
    ctx.fillText('林简 · VERIFIED NPC', NPC_X, GROUND_Y + 34);
  }
  ...
  Promise.all(framePaths.map((path) => new Promise((resolve) => {
    ...
    npcFrames.push(...images);
    state.assetsReady = images.every((image) => image.naturalWidth > 0);
    if (!state.assetsReady) announce('SYSTEM', 'NPC 四帧加载失败，当前使用程序化占位角色。');
    draw();
  });
